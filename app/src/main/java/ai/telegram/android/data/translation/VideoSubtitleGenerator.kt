...
class VideoSubtitleGenerator(
    ...
    private suspend fun generateSourceTranscript(
        ...
        val sourceCues = speech.toSourceCues(durationMs)
...
private data class RecognizedSpeech(
    val text: String = "",
...
private fun RecognizedSpeech.toSourceCues(durationMs: Long): List<VideoSubtitleCue> {
    val timedWords = words
    ...
    return timedWords?.toTimedCues(durationMs)
        ?: splitSentences(text).toDistributedCues(durationMs)
}
...
private fun String.cleanSubtitleText(): String {
